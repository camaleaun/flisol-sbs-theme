...
class Controller_Atividades {

    public function __construct(){
        add_action( 'pre_get_posts', array( $this, 'setPostsPerPage' ), 10 ,2 );
        add_filter( 'get_atividades_post' , array( $this , 'get_atividades_post') );
        add_filter( 'get_atividades_posts' , array( $this , 'get_atividades_posts') );
    ...
    public function setPostsPerPage($query) {
        ...
        if( $query->get( 'post_type' ) !=  'atividades') {
            return;
        }
    ...
    public function get_post_by_id( $post_id ) {
        ...
        if( $post ) {
            return $this->get_atividades_post( $post );
        }
        return false;
    }

    public function get_atividades_post( $post ) {
    ...
    public function get_atividades_posts( $posts ) {
        if( count( $posts ) == 0 ) {
            return null;
        }
        ...
        foreach( $posts as $post ) {
            $posts_blog[] = $this->get_atividades_post( $post );
        }
        return $posts_blog;
    }
    ...
    public function get_last_post_home() {
        ...
        $this->get_atividades_post( $post_blog );
    ...
    public function get_last_post_sidebar() {
       ...
       $posts = get_posts($args);
        if ( $posts ) {
            foreach ( $posts as $post ) {
                $list_posts[] = $this->get_atividades_post($post);
            }
            return $list_posts;
        }
        return false;
    ...
    public function get_posts_related( $post_id, $taxonomy ) {
        ...
        if ( $posts ) {
            foreach ( $posts as $post_op ) {
                $list_posts[] = $this->get_atividades_post($post_op);
            }
            return $list_posts;
        }
        return false;
